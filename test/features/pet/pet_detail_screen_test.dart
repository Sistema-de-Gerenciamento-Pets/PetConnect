...
Widget _appPara(Pet? pet, {AsyncValue<Pet?>? estadoForcado}) {
  final router = GoRouter(
    ...
    routes: [
      ...
      GoRoute(
        path: '/pet/:id/editar',
        builder: (context, state) => const Scaffold(body: Text('Tela Editar')),
      ),
      GoRoute(
        path: '/pet/:id/vacinas',
      ...
      GoRoute(
        path: '/configuracoes',
        builder: (context, state) =>
            const Scaffold(body: Text('Tela Configurações')),
      ),
    ],
  );
...
void main() {
  group('PetDetailScreen — layout em cards', () {
    ...
    testWidgets('editar e excluir continuam acessíveis como antes',
        (tester) async {
      await tester.pumpWidget(_appPara(_petCompleto));
      await tester.pumpAndSettle();

      expect(find.widgetWithText(ElevatedButton, 'EDITAR'), findsOneWidget);
      expect(find.widgetWithText(OutlinedButton, 'EXCLUIR'), findsOneWidget);
    });
