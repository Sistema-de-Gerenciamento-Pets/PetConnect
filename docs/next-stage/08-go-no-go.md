...
| Infraestrutura | ⚠️ Decisões de hospedagem (MongoDB Atlas, host da API, página do QR) **ainda pendentes** — matriz pronta, nada criado | Restrição |
...
1. **Não abrir a página pública do QR pro público real** até a FASE 9
   ter hospedagem decidida — hoje ela nem existe.
...
3. **Rodar o `device-validation-checklist.md` antes de considerar
   ...
   automatizados verdes, nenhuma tela foi confirmada visualmente ainda
   nesta etapa.
...
estiverem preenchidas com `PASS`, e (b) o round-trip do Cloudinary for
confirmado com a secret real, e (c) a decisão de acesso entre repos for
tomada (mesmo que a solução ainda não esteja implementada, só decidida) —
