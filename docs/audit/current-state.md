...
|---|---|---|---|
| **R-01** | Sem acesso aos **dados reais** do Firestore nem à contagem de documentos → impossível fazer o relatório de qualidade de dados (etapa 50) e dimensionar a migração | 🟡 Média (era 🔴) | ✅ Mitigado: `tools/firestore-backup/` faz backup + relatório via Admin SDK (somente leitura, **não exige Blaze**). `gcloud firestore export` foi descartado por exigir faturamento. Pendente: usuário gerar chave de conta de serviço, rodar `npm run backup` e colar `report.md`. **Não migrar dados antes disso.** |
| **R-02** | **Firestore Rules deployadas são muito permissivas** — confirmado: catch-all `allow read` (banco inteiro legível **sem login**, incl. PII de 18 usuários), `Pets` leitura pública, `Localizacoes` **escrita pública**. Ver [`../security/firestore-rules-deployed.md`](../security/firestore-rules-deployed.md) | 🔴 Alta | Endurecimento **faseado** (não quebrar o app antigo que os 18 usuários usam): versionar `firestore.rules`; cortar leitura anônima cedo (após FASE 3); `Localizacoes write:false` quando a API assumir; `read,write:false` nas coleções migradas na FASE 11 |
